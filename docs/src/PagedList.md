...
[the default instance](Configuration.md#default-instance) on first access, so
...
Typed paths like `Tables.Company_.name` are generated automatically by the
Stormify Gradle plugin. For each entity class `Foo` the plugin emits a `Foo_`
entry inside the `Tables` object under the `onl.ycode.stormify.generated`
package, with fields for each scalar property and nested objects for FK
references. Override the package and class names via the `stormify { }` block
— see [Installation](Installation.md#configuration).
