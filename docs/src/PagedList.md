...
Typed paths like `Company_.name` are generated by the `annproc` KSP processor —
the same processor that generates entity metadata. Add it once to your build as
described in
[Installation › KSP Setup](Installation.md#ksp-setup).

For each entity class `Foo` the processor emits a `Foo_` object under the
`onl.ycode.stormify.generated` package, with fields for each scalar property and
nested objects for FK references. The package name is overridable via the
`stormify.generatedPackage` KSP option — see
[Annotations › Customizing the generated package](Annotations.md#customizing-the-generated-package-and-class-names).
On Native/Android/iOS, `annproc` is **required** anyway (for entity metadata) —
you get typed paths for free.
