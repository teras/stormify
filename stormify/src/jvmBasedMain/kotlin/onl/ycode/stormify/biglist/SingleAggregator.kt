...
/**
 * JVM / Android actual of [SingleAggregator]. Adds a Java-friendly
 * `execute(Class<R>)` overload on top of the common API so Java callers
 * can write `agg.sum(path).execute(BigDecimal.class)`.
 */
actual class SingleAggregator internal actual constructor(
    ...
    /**
     * Java-friendly overload of [execute] that accepts a [Class] instead of a
     * Kotlin [KClass]. Delegates to the common implementation.
     */
    fun <R : Any> execute(type: Class<R>): R? = core.executeSingle(type.kotlin)
}
