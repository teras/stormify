...
open class CrudTableTest {
    ...
    fun crudTableDirectCallsAutoCommit() = withDb("CT-DIRECT") { s ->
        ...
        val row = s.readOne<Row>("SELECT * FROM ct_test WHERE id = 1")!!
    ...
    fun crudTableJoinsActiveTransaction() = withDb("CT-TX") { s ->
        ...
        s.transaction {
            Row(10, "A").create()
            Row(11, "B").create()
            val fetched = s.readOne<Row>("SELECT * FROM ct_test WHERE id = 10")!!
        ...
        assertFailsWith<RuntimeException> {
            s.transaction {
                ...
                assertNotNull(s.readOne<Row>("SELECT * FROM ct_test WHERE id = 20"))
                throw RuntimeException("abort")
            }
        }
        assertNull(s.readOne<Row>("SELECT * FROM ct_test WHERE id = 20"))
    }
    ...
    fun crudTableJoinsNestedSavepoint() = withDb("CT-NESTED") { s ->
        ...
        s.transaction {
            ...
            assertNull(s.readOne<Row>("SELECT * FROM ct_test WHERE id = 31"))
            assertNotNull(s.readOne<Row>("SELECT * FROM ct_test WHERE id = 30"))
        }
        assertNotNull(s.readOne<Row>("SELECT * FROM ct_test WHERE id = 30"))
        assertNull(s.readOne<Row>("SELECT * FROM ct_test WHERE id = 31"))
    }
